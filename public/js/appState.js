...
export const AppState = {
  ...
  recreateShape(shapeData) {
    ...
    if (shapeElement) {
      ...
      try { shapeElement.style.backgroundColor = color; } catch (e) { console.debug('No se pudo aplicar color al elemento:', e); }
    }
  ...
  recreateShapeInWorkspace(shapeData, tabId) {
    ...
    if (shapeElement) {
      ...
      try { shapeElement.style.backgroundColor = color; } catch (e) { console.debug('No se pudo aplicar color al elemento en workspace:', e); }
    }
  ...
  saveCanvas() {
    ...
    if (typeof window.showNotification === 'function') window.showNotification('Diagrama guardado exitosamente');
  },
  ...
  loadCanvas() {
    if (window.storage?.importFromJSON) {
      window.storage.importFromJSON((loadedShapes, error) => {
        if (error) {
          if (typeof window.showNotification === 'function') window.showNotification('Error al cargar el archivo', true);
          return;
        }
        document.querySelectorAll('.action-shape').forEach((shape) => shape.remove());
        this.actionsShapes = loadedShapes;
        this.actionsCounter = this.actionsShapes.length ? Math.max(...this.actionsShapes.map((s) => s.id)) + 1 : 0;
        this.actionsShapes.forEach((shapeData) => {
          if (shapeData.tabId) this.recreateShapeInWorkspace(shapeData, shapeData.tabId);
          else this.recreateShape(shapeData);
        });
        window.configPanel?.close?.();
        this.saveState();
        if (typeof window.showNotification === 'function') window.showNotification('Diagrama cargado exitosamente');
      });
    } else {
      ...
      input.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => {
          try {
            ...
            if (typeof window.showNotification === 'function') window.showNotification('Error al leer el archivo JSON', true);
          }
        };
  ...
  exportCanvas() {
    if (typeof window.showNotification === 'function') {
      window.showNotification('Función de exportar a imagen en desarrollo.\nPor ahora usa "Guardar" para exportar en formato JSON.');
    }
  },
